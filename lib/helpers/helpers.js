...
queryize = function (actionset) {
  ...
  var sublimit = actionset.query().sublimit ? parseInt(actionset.query().sublimit) : 0 
  if (sublimit || !sublimit) {
    return Mongo.Collection.get(collection, { connection: connection }).find(selector, {sort: sort, limit: sublimit}).fetch()
  } else {
    sweetAlert("was no sublimit", selector)
    return Mongo.Collection.get(collection, { connection: connection }).find(selector, {sort: sort}).fetch()
  }
}

preventDefault = (fn, ...args) => (e) => {
  e.preventDefault()
  fn(e, ...args)
}

dateFormat = function(date, format) {
  var time = timestamptomoment(date).format(format)
  return time
} 
